...
//使用8*8的microkernel
__global__ void sgemm_fast_kernel_optimiz_3(int k, int m, int n,
    float* d_A, float* d_B, float* d_C)
{
    ...
    for (int po = 0; po < k; po += KERNEL_SIZE)
    {
        ...
        sm_A[tx * REG_TILE_SIZE + 4][ty] = d_A(Ci + 4, po + ty);
        sm_A[tx * REG_TILE_SIZE + 5][ty] = d_A(Ci + 5, po + ty);
        sm_A[tx * REG_TILE_SIZE + 6][ty] = d_A(Ci + 6, po + ty);
        sm_A[tx * REG_TILE_SIZE + 7][ty] = d_A(Ci + 7, po + ty);
        ...
        for (int pi = 0; pi < KERNEL_SIZE; pi++)
        {
            ...
            reg_a4 = sm_A[tx * REG_TILE_SIZE + 4][pi];
            reg_a5 = sm_A[tx * REG_TILE_SIZE + 5][pi];
            reg_a6 = sm_A[tx * REG_TILE_SIZE + 6][pi];
            reg_a7 = sm_A[tx * REG_TILE_SIZE + 7][pi];
	...
	float* C, int ldc)
{
    ...
    sgemm_fast_kernel_optimiz_3 << <dim_block, dim_thread >> > (k, m, n,
        d_A, d_B, d_C);

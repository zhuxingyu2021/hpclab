...
//通过在载入矩阵A时做转置，减少shared memory bank conflict
__global__ void sgemm_fast_kernel_optimiz_4(int k, int m, int n,
    float* d_A, float* d_B, float* d_C)
{
    ...
    {
        sm_A[ty][tx * REG_TILE_SIZE + 0] = d_A(Ci + 0, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 1] = d_A(Ci + 1, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 2] = d_A(Ci + 2, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 3] = d_A(Ci + 3, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 4] = d_A(Ci + 4, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 5] = d_A(Ci + 5, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 6] = d_A(Ci + 6, po + ty);
        sm_A[ty][tx * REG_TILE_SIZE + 7] = d_A(Ci + 7, po + ty);
	...
	float* C, int ldc)
{
    ...
    sgemm_fast_kernel_optimiz_4 << <dim_block, dim_thread >> > (k, m, n,
        d_A, d_B, d_C);

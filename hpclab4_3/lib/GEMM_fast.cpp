...
typedef struct{
    ...
    float* B; int ldb;
    float* C; int ldc;
}gemm_fast_multiply_aligned_single_thread_kernel_args;
void* gemm_fast_multiply_aligned_single_thread_kernel(void* pf_args)
{
    int k = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->k;
    int m = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->m;
    int n = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->n;
    int lda = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->lda;
    int ldb = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->ldb;
    int ldc = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->ldc;
    float* A = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->A;
    float* B = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->B;
    float* C = PF_GET_PARG(pf_args, gemm_fast_multiply_aligned_single_thread_kernel_args)->C;
    int io;

    for (io = PF_GET_INDEX_START(pf_args); io < PF_GET_INDEX_END(pf_args); io += PF_GET_INDEX_INCREMENT(pf_args)) {
        int BMsz = MIN(m - io, BM);
        for (int p = 0; p < k; p += BK) {
            int BKsz = MIN(k - p, BK);
            float *packed_A = (float *) aligned_malloc(sizeof(float) * BMsz * BKsz, GEMM_CACHELINE_SIZE);
            for (int jo = 0; jo < n; jo += BN) {
                int BNsz = MIN(n - jo, BN);
                float *packed_B = (float *) aligned_malloc(sizeof(float) * BKsz * BNsz, GEMM_CACHELINE_SIZE);
                for (int ii = 0; ii < BMsz; ii += 8) {
                    if (jo == 0) packA(BKsz, packed_A + BKsz * ii, &A(io + ii, p), lda);
                    for (int ji = 0; ji < BNsz; ji += 8) {
                        if (ii == 0) packB(BKsz, packed_B + BKsz * ji, &B(p, jo + ji), ldb);
                        gemm_kernel_8x8_alignedC(BKsz, packed_A + BKsz * ii, lda, packed_B + BKsz * ji, ldb,
                                                 &C(io + ii, jo + ji), ldc);
                    }
                }
...
typedef struct{
    ...
    bool woaligned_B;
    bool woaligned_C;
}gemm_fast_multiply_woaligned_single_thread_kernel_args;
void* gemm_fast_multiply_woaligned_single_thread_kernel(void* pf_args)
{
    int k = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->k;
    int m = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->m;
    int n = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->n;
    int lda = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->lda;
    int ldb = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->ldb;
    int ldc = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->ldc;
    float* A = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->A;
    float* B = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->B;
    float* C = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->C;
    bool woaligned_B = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->woaligned_B;
    bool woaligned_C = PF_GET_PARG(pf_args, gemm_fast_multiply_woaligned_single_thread_kernel_args)->woaligned_C;
        ...
        for (int p = 0; p < k; p += BK) {
            ...
            for (int jo = 0; jo < n; jo += BN) {
                ...
                for (int ii = 0; ii < BMsz; ii += 8) {
                    ...
                    for (int ji = 0; ji < BNsz; ji += 8) {
                        if (ii == 0){
                            if(woaligned_B) packB_woaligned(BKsz, packed_B + BKsz * ji, &B(p, jo + ji), ldb);
                            else packB(BKsz, packed_B + BKsz * ji, &B(p, jo + ji), ldb);
                        }
                        if(!woaligned_C) gemm_kernel_8x8_alignedC(BKsz, packed_A + BKsz * ii, lda, packed_B + BKsz * ji, ldb, &C(io + ii, jo + ji), ldc);
...
typedef struct{
    ...
    float* B; int ldb;
    float* C; int ldc;
}gemm_fast_multiply_wo_factor8_single_thread_kernel_args;
void* gemm_fast_multiply_wo_factor8_single_thread_kernel(void* pf_args)
{
    int k = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->k;
    int m = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->m;
    int n = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->n;
    int lda = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->lda;
    int ldb = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->ldb;
    int ldc = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->ldc;
    float* A = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->A;
    float* B = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->B;
    float* C = PF_GET_PARG(pf_args, gemm_fast_multiply_wo_factor8_single_thread_kernel_args)->C;
...
void sgemm_fast(int k, int m, int n,
    ...
    if ((m % 8 == 0) && (n % 8 == 0)) {
        ...
        if((size_t)B % GEMM_CACHELINE_SIZE) aligned_B = false;
        if((size_t)C % GEMM_CACHELINE_SIZE) aligned_C = false;

        if(aligned_B && aligned_C) {
            gemm_fast_multiply_aligned_single_thread_kernel_args arg;
            arg.k = k; arg.m = m; arg.n = n;
            arg.A = A; arg.lda = lda;
            arg.B = B; arg.ldb = ldb;
            arg.C = C; arg.ldc = ldc;

            parallel_for(0, m, BM, &gemm_fast_multiply_aligned_single_thread_kernel, (void*)&arg, N_THREADS);

            //gemm_fast_multiply_aligned(k, m, n, A, lda, B, ldb, C, ldc);
        }
        else {
            gemm_fast_multiply_woaligned_single_thread_kernel_args arg;
            arg.k = k; arg.m = m; arg.n = n;
            arg.A = A; arg.lda = lda;
            arg.B = B; arg.ldb = ldb;
            arg.C = C; arg.ldc = ldc;
            arg.woaligned_B = aligned_B;
            arg.woaligned_C = aligned_C;

            parallel_for(0, m, BM, &gemm_fast_multiply_woaligned_single_thread_kernel, (void*)&arg, N_THREADS);

            //gemm_fast_multiply_woaligned(k, m, n, A, lda, B, ldb, C, ldc, aligned_B, aligned_C);
        }
    }
    else {
        gemm_fast_multiply_wo_factor8_single_thread_kernel_args arg;
        arg.k = k; arg.m = m; arg.n = n;
        arg.A = A; arg.lda = lda;
        arg.B = B; arg.ldb = ldb;
        arg.C = C; arg.ldc = ldc;

        parallel_for(0, m, BM, &gemm_fast_multiply_wo_factor8_single_thread_kernel, (void*)&arg, N_THREADS);

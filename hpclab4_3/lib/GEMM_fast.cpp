...
typedef struct{
    ...
    float* B; int ldb;
    float* C; int ldc;
}sgemm_fast_single_thread_kernel_args;
void* sgemm_fast_single_thread_kernel(void* pf_args)
{
    // 读取传入参数
    int k = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->k;
    int m = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->m;
    int n = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->n;
    int lda = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->lda;
    int ldb = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->ldb;
    int ldc = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->ldc;
    float* A = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->A;
    float* B = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->B;
    float* C = PF_GET_PARG(pf_args, sgemm_fast_single_thread_kernel_args)->C;
                ...
                float* C, int ldc, int n_threads)
{
    ...
    // 准备子线程传入参数
    sgemm_fast_single_thread_kernel_args arg;
    arg.k = k; arg.m = m; arg.n = n;
    arg.A = A; arg.lda = lda;
    arg.B = B; arg.ldb = ldb;
    arg.C = C; arg.ldc = ldc;

    parallel_for(0, m, BM, &sgemm_fast_single_thread_kernel, (void*)&arg, n_threads);
}
